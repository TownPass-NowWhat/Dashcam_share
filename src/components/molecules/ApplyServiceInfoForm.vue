        ...
        <div class="overflow-hidden flex flex-col gap-4 pt-5">
          ...
          <div v-for="(item, index) in formFormat.data.form_format" :key="index">
            ...
            <div v-else-if="item.type === 'select'" class="flex flex-col">
              <label :for="item.field" class="field-label font-semibold">{{ item.label }}<span v-if="item.required">*</span></label>
              <BaseSelect v-model="form[item.field]" :selectId="item.field" :required="item.required" :options="item.options" :triggerValidate="props.triggerValidate" :default-selected="item.default_option" class="w-full" />
            </div>
            <!-- type = textarea -->
            <div v-else-if="item.type === 'textarea'" class="flex flex-col">
              <label :for="item.field" class="field-label font-semibold">{{ item.label }}<span v-if="item.required">*</span></label>
              <BaseTextarea v-model="form[item.field]" :id="item.field" :label="item.label" :required="item.required" :triggerValidate="props.triggerValidate" class="w-full" placeholder="100字以內" />
            </div>
            <!-- type = radio_group -->
            <!-- <div v-else-if="item.type === 'radio_group'" class="flex flex-col">
              <p class="field-label font-semibold">{{ item.label }}<span v-if="item.required">*</span></p>
              <div class="flex flex-wrap gap-4">
                <BaseRadio v-model="form[item.field]" v-for="option in item.options" :key="option.value" :radioId="option.value" :radioName="item.field" :radioText="option.label" :required="item.required" :triggerValidate="props.triggerValidate" />
              </div>
            </div> -->
            <!-- type = checkbox_group -->
            <!-- <div v-else-if="item.type === 'checkbox_group'" class="flex flex-col">
              ...
              <div class="flex flex-wrap gap-1.5">
                <BaseCheckbox v-for="option in item.options" :key="option.value" :option="option" :required="item.required" :triggerValidate="props.triggerValidate" v-model="form[item.field]" />
              </div>
            </div> -->
            <!-- type = upload -->
